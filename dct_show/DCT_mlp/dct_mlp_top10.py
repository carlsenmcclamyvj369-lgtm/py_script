...
class DCTClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(10, 1),
            # nn.ReLU(),
            # nn.Linear(8, 1)  # 最后一层依然输出 Logits，不用 Sigmoid
        )
...
def load_dataset_features(dataset_dir="dataset"):
    """
    遍历 clean 和 mosquito_noise 文件夹，提取 10维 核心特征
    """
    X_list = []
    Y_list = []
        ...
        for img_path in img_paths:
            ...
            for by in range(0, H - 7, 8):
                for bx in range(0, W - 7, 8):
                    ...
                    abs_dct[0, 0] = 0.0
                    feat_64 = np.log1p(abs_dct).flatten()

                    # 【核心剪枝逻辑】仅提取 10 个最关键的物理频段
                    feat_10 = feat_64[TOP_10_INDICES]
...
# 3. 核心评估与训练函数
# ==========================================
def train_and_evaluate_mlp(dataset_dir="dataset", model_save_path="dct_mlp_top10_weights.pth", epochs=150):
    print(">>> 正在从数据集中提取 Top 10 DCT 特征...")
    ...
    print(f"特征提取完毕！共获得 {len(X_data)} 个 10维 样本。")
    ...
    print(f"\n[策略调整] 启用正样本权重加持: {weight_ratio:.2f} 倍惩罚")

    criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight_tensor)
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    print(f"\n>>> 开始训练剪枝模型 (训练集: {len(X_train)}，测试集: {len(X_test)})...")
    model.train()
    ...
    print(f"✅ 训练完成！新模型权重已保存至: {model_save_path}")

    # 【盲测评估报告】
    print("\n" + "=" * 50)
    print(">>> 剪枝后模型盲测评估报告 (测试集未参与训练):")
    model.eval()
    with torch.no_grad():
        ...
        print(f"真负类(正确放过树枝): {tn:<5} | 假正类(误杀树枝): {fp}")
        print(f"假负类(漏掉文字噪声): {fn:<5} | 真正类(正确抓出噪声): {tp}")

        recall_percent = (tp / (tp + fn)) * 100 if (tp + fn) > 0 else 0
        print(f"\n⚡ 当前蚊子噪声召回率 (Recall): {recall_percent:.1f}%")
    ...
    # 【提取部署参数】
    print("=" * 50)
    print(">>> 硬件 C++ 部署参数提取 (Top 10 Features):")
    ...
    print(f"float weights[10] = {weights_str};")
    print("=" * 50 + "\n")
...
def infer_full_image(image_path, model):
    ...
    for by in range(0, H - 7, 8):
        ...
        for bx in range(0, W - 7, 8):
            ...
            # 【推理时剪枝】确保推理时也只喂给模型这 10 个特征
            feat_10 = feat_64[TOP_10_INDICES]

            batch_features.append(feat_10)
            x_coords.append(bx)
...
if __name__ == "__main__":
    # 使用全新的 pth 命名
    MODEL_PATH = "dct_mlp_top10_weights.pth"

    # 1. 训练与盲测评估
    trained_model = train_and_evaluate_mlp(dataset_dir="dataset", model_save_path=MODEL_PATH, epochs=200)
    ...
    if trained_model is not None:
        ...
        if img_bgr is not None:
            ...
            cv2.namedWindow("MLP ROI Prediction (Top 10 Pruned)", cv2.WINDOW_NORMAL)
            cv2.imshow("MLP ROI Prediction (Top 10 Pruned)", combined)
            print("按任意键退出...")
            cv2.waitKey(0)
